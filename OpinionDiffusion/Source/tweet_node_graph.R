...
data = read.csv("../Data/KeralaFloods.csv")
# data = head(data)
# str(data)
print(head(data))
...
create_network = function(selected_tweets)
{
  ...
  nodes = data.frame(Tweet_id = character(), Tweet_text = character(), Tweet_time = character(), Original_tweeter = character(), Retweeter_name = character(), User_type = character())
  ...
  for (index in 1:nrow(selected_tweets))
  {
    ...
    #Created time
    tweet_time = tweet_data[["created"]]
    
    #Tweet_ID
    tweet_id = tweet_data[["X"]]
    
    if (tweet_words[1] == "RT")
    {
      nodes = rbind(nodes, data.frame("Original_user" = "Retweeter_name" = tweet_user, "Node_type" = "Tweet", "Tweet_time" = tweet_time))
    ...
    if (tweet_words[1] == "RT")
    {
      ...
      nodes = rbind(nodes, data.frame("Tweet_id" = tweet_id, "Original_user" = original_user, "Retweeter_name" = tweet_user, "Node_type" = "Tweet", "Tweet_time" = tweet_time))
      
      # Add original and retweet poster screenNames to Edge List
      tweeters = c(tweeters, tweet_id)
